...
listeners:
  - listener_id: CLISTENER-TFS-001
    ...
    surface_kind: <network | kubernetes_resource | filesystem | object_store | webhook | controller | database | library_api>
    protocol: <HTTP | REST | gRPC | TCP | WebSocket | SSE | Kubernetes | filesystem | S3 | SQL | library | other>
    bind_address: <address | not_applicable | unknown>
    port: <number | not_applicable | unknown>
    locator: <socket, CRD, path, bucket contract, database, or symbol>
...
interfaces:
  - interface_id: CIFACE-TFS-001
    ...
    interface_family: <API | Dashboard | Runtime | Model Serving | Artifact | Storage | Telemetry | Streaming | Management | Auth/Admin | Proxy | Orchestration / Kubernetes Control>
    interface:
      name: Predict
      path_or_rpc: tensorflow.serving.PredictionService/Predict
      method: <GET | POST | RPC | reconcile | read | write | other>
    ...
    security_contract:
      ...
      built_in_authz: <object | tenant | namespace | role | coarse | absent | externalized | unknown>
      ...
      expected_resource_scope: <object | path | bucket | database | namespace | cluster | process | library-caller | not_applicable | unknown>
    protection_assumption:
      value: <self-protected | externally-protected | trusted-internal | localhost-only | caller-enforced | unknown>
...
coverage:
  ...
  discovery_checks:
    - check_id: CCHECK-TFS-001
      check: <listener_discovery | router_service_discovery | handler_rpc_enumeration | dynamic_registration | proto_services | feature_config_flags | non_network_surfaces | reverse_search | cross_check>
...
- `coverage_status` is `incomplete` whenever an entire protocol, plugin registry, route stack,
  configuration profile, or non-network registration mechanism remains unresolved.
- Every failed discovery check references at least one structured residual gap. Every residual gap
  identifies affected IDs or an affected protocol/surface/profile/source root so Stage B can
  propagate it.
