...
description: "Analyze security deltas introduced when a platform integrates a third-party AI/ML component. Use with a Stage A Component Interface Baseline plus platform source and deployment configuration to resolve interface bindings, evidence-backed reachability, identity and policy propagation, plane/protocol/storage/privilege deltas, candidate vulnerabilities, manual verification, root-cause ownership, and CVE triage for GPU/AI platforms, MLOps, model serving, notebooks, workflows, telemetry, storage, dashboards, controllers, and management surfaces."
---
...
3. platform deployment evidence such as Helm values, rendered YAML, operators, gateways, service
   mesh policies, RBAC, and network policy.
...
The resolver must map platform objects and routes to existing `CIFACE-*` IDs. A Service, Ingress, or
open port is not proof that every listener operation is reachable. Build the full evidence chain to
the concrete component interface.
...
Do not retain a candidate merely because the component interface is dangerous. Require an
...
Use A1–A8 unchanged as the candidate taxonomy. Do not create a new category just to describe a new
surface. `Orchestration / Kubernetes Control` is an interface family and `surface_kind` is evidence,
not a new risk taxonomy.
